...
public:
    ...
    int filterRowStart{INT_MIN};
    int filterRowEnd{INT_MAX};
...
void FilterProxy::setFilterRowStart(int filterRowStart)
{
    if (d->filterRowStart != filterRowStart) {
        d->filterRowStart = filterRowStart;
        Q_EMIT filterRowStartChanged();
    }
}

int FilterProxy::filterRowStart() const
{
    return d->filterRowStart;
}

void FilterProxy::setFilterRowEnd(int filterRowEnd)
{
    if (d->filterRowEnd != filterRowEnd) {
        d->filterRowEnd = filterRowEnd;
        Q_EMIT filterRowEndChanged();
    }
}

int FilterProxy::filterRowEnd() const
{
    return d->filterRowEnd;
}
...
bool FilterProxy::filterAcceptsRow(int sourceRow, const QModelIndex &sourceParent) const
{
    ...
    if (index.row() >= d->filterRowStart && index.row() <= d->filterRowEnd) {
        if (d->filterBoolean) {
